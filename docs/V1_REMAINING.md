...
### Phase A — AI boundary (core, no runtime). **DONE.** Blocks ai docs + API freeze.
...
### Phase B — Operations commands. **DONE.** Independent of A.
...
- 2026-06-19: Branch `feat/ai-handoff-boundary`. **Phase A complete** (A1–A8)
  via TDD — AI session persistence, `ai` RouteDecision + ConversationRelayConfig,
  Connect/ConversationRelay TwiML, `AiSessionHandler` contract (disabled
  default), router + inbound wiring (persists session, emits `AiSessionStarted`),
  and the `/ai/status` callback (`AiSessionStatusProcessor` → `AiSessionEnded`/
  `AiSessionFailed`, idempotent). Suite 80 passing, pint clean. Committed.
- 2026-06-19: **Phase B complete** (B1–B3) via TDD — `PruneWebhookReceipts` job +
  `phone:prune`, real reprocessing in `WebhookReplayService` + `phone:webhook:replay`,
  and `phone:install`. All four commands registered. Suite 86 passing, pint clean.
  Committed. Next: **Phase C** docs pass (`C1`), then `D1` event-name reconciliation,
